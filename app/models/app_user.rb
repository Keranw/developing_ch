class AppUser < ApplicationRecord
  ...
  def self.user_profile_complete(params)
    ...
    @aim_user[:sex] = params[:sex].to_i
    @aim_user[:not_interest] = params[:sex]
    image_count = 0
    # 注册的时候可以上传几张？
    # 注意这些image并不是avatar，avatar是指定的一张
    ...
    ############################################################
      AppUser.save_raw(image_count, params[:user_id], f[:first_frame], f[:content])
    #############################################################
    end
    ...
    #################################################################################
    temp = AppUser.save_raw(0, params[:user_id], params[:first_frame], params[:avatar])
    ##################################################################################
    #注意现在动图和静图存储的是json的键
...
################################################################
  def self.save_raw(count, user_id, image_bits, video_bits)
    current_time = Time.now.to_i.to_s + count.to_s
    image_key = current_time + ".jpg"
    video_key = current_time + ".mp4"
    @temp_aim_user = AppUser.find_by(user_id:user_id.to_i)
    @temp_aim_user[:temp_image][image_key] = image_bits
    @temp_aim_user[:temp_video][video_key] = video_bits
    @temp_aim_user[:avatar_count] += 1
    @temp_aim_user.save!
    [image_key, video_key]
  end
###############################################################
  ...
  def self.fetch_my_info(aim_user_id)
    ...
    #####################################################################
    temp = @aim_user[:avatar_frame]
    result[:first_frame_name] = temp
    result[:first_frame] = @aim_user[:temp_image][temp]
    #####################################################################
  ...
  def self.fetch_my_picture_wall(user_id)
    ...
    ##Find.find(path).each do |f|
      ...
      ##end
    ##end
    @current_user[:temp_image].each do |k,v|
      images << {"picture_name":k.to_s.split('.').first, "first_frame":v}
    end
    result = {}
    ...
    ###############################################################################
    result[:avatar_name] = @current_user[:avatar]
    result[:rest_five_names] = @current_user.pairing_info[:rest_five]
    ###############################################################################
    result
  end
  ...
  def self.fetch_the_dynamic_image(user_id, image_name)
    ...
    ##############################################
    @temp_user = AppUser.find_by(user_id: user_id.to_i)
    video_key = image_name + ".mp4"
    @temp_user[:temp_video][video_key]
    ##############################################
  end
  ...
  def self.update_my_avatar(user_id, video_name, image_name)
    ...
    #########################################
    @aim_user[:avatar] = video_name
    @aim_user[:avatar_frame] = image_name
    #########################################
    #再见再也不见
...
=end
  ####################################
  @aim_pairing_info = PairingInfo.includes(:app_user).find_by(user_id:user_id)
  image_names.each do |f|
    if @aim_pairing_info[:rest_five].include?(f)
      @aim_pairing_info[:rest_five].delete(f)
    end
    image_key = f + ".jpg"
    video_key = f + ".mp4"
    @aim_pairing_info.app_user[:temp_image].delete(image_key)
    @aim_pairing_info.app_user[:temp_video].delete(video_key)
  end
  @aim_pairing_info.save!
  @aim_pairing_info.app_user.save!
  ####################################
  end
end
